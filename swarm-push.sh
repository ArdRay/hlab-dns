...
ssh -q -o 'StrictHostKeyChecking=no' $SSH_USER@$HOST 'docker login -u _json_key --password-stdin https://eu.gcr.io < ~/json.key && docker stack deploy --compose-file docker-stack.yml dns'
